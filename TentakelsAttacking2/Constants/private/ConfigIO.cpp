...
void LoadConfig() {
	...
	if (nlohmann::json global; loadSection(load, global, ConfigTypes::GLOBAL, constants.global.configEntryCount)) {
		if (std::string out; loadString(global, out, ConfigTypes::CURRENT_LANGUAGE_NAME)) { constants.global.current_language_name = out; }
...
void SaveConfig() {
	...
	save[CToS(ConfigTypes::GLOBAL)] = {
		{ CToS(ConfigTypes::CURRENT_LANGUAGE_NAME), constants.global.current_language_name },
